  ...
  <script>
    ...
    async function loadDashboard() {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          charts.bar = new Chart(barCanvas, { type: 'bar', data: { labels, datasets: [{ data: values, backgroundColor: ['#3b82f6','#10b981','#f59e0b','#8b5cf6'] }] }, options: opts });
          charts.line = new Chart(lineCanvas, { type: 'line', data: { labels, datasets: [{ data: values, tension: 0.4, borderColor: '#3b82f6' }] }, options: opts });
          charts.pie = new Chart(pieCanvas, { type: 'pie', data: { labels, datasets: [{ data: values, backgroundColor: ['#3b82f6','#10b981','#f59e0b','#8b5cf6'] }] }, options: { ...opts, plugins: { legend: { position: 'right' } } } });
        } catch (err) {
    ...
    async function fetchCustomers() {
      ...
      try {
        ...
        showToast("Error clientes", "error");
      }
    }
    ...
    loadDashboard();
      ...
      if(url.pathname==="/api/dashboard"){
        ...
        } catch (e) { console.log("Error ingresos:", e); }
        ...
        } catch (e) { console.log("Error active rentals:", e); }

        try {
          // ← Corrección clave aquí: TRIM + LOWER para evitar problemas de formato
          ...
          console.log("Botes disponibles detectados:", available_boats);  // ← Log para verificar
        } catch (e) { console.log("Error boats available:", e); }

        try {
          const customers = await env.DB.prepare("SELECT COUNT(*) as total FROM customers").first();
          total_customers = customers.total ?? 0;
        } catch (e) { console.log("Error customers:", e); }
