...
<body>
  ...
  <div id="reservationModal" class="modal-overlay">
    <div class="modal" style="width:520px">
      ...
      <div class="form-group">
        ...
        <input id="duration" type="number" step="0.5" min="0.5">
      </div>
  ...
  <script>
    ...
    // ... (resto de funciones de clientes: openCustomerModal, editCustomer, saveCustomer, deleteCustomer sin cambios)
    ...
    // Facturación – con nombre del cliente
    ...
    loadDashboard();
      ...
      if (url.pathname.startsWith("/api/reservations")) {
        ...
        if (request.method === "POST") {
          const body = await request.json();
          await env.DB.prepare("INSERT INTO reservations (boat_id, customer_id, start_time, end_time, status) VALUES (?,?,?,?, 'pendiente')")
        ...
        if (request.method === "PUT") {
          ...
          const id = parts[parts.length-1];
          const body = await request.json();
          await env.DB.prepare("UPDATE reservations SET boat_id=?, customer_id=?, start_time=?, end_time=?, status=? WHERE id=?")
