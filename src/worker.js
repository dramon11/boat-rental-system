...
.table-container{margin-top:20px;background:white;padding:20px;border-radius:14px;box-shadow:0 6px 20px rgba(0,0,0,0.05);}
...
.input-search{padding:8px;border:1px solid #ccc;border-radius:6px;width:200px;}
.toast{position:fixed;bottom:20px;right:20px;background:#333;color:white;padding:12px 20px;border-radius:8px;opacity:0;transition:0.3s;}
...
<div class="menu-item" onclick="showCustomers()">Clientes</div>
</div>

<div class="header">
<div id="headerTitle">Panel Administrativo</div>
<div>Admin</div>
</div>
...
<div id="customersModule" style="display:none;">
  <div class="table-container">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
      ...
      <button class="btn btn-primary" onclick="openCustomerModal()">+ Nuevo Cliente</button>
    </div>
...
<!-- MODAL CLIENTES -->
<div id="customerModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);justify-content:center;align-items:center;">
  <div style="background:white;padding:20px;border-radius:10px;max-width:400px;width:100%;">
    <h3>Nuevo Cliente</h3>
    <input id="name" placeholder="Nombre completo" style="width:100%;margin:5px 0;padding:8px;"/>
    <input id="doc" placeholder="Documento" style="width:100%;margin:5px 0;padding:8px;"/>
    <input id="phone" placeholder="Teléfono" style="width:100%;margin:5px 0;padding:8px;"/>
...
function showDashboard(){
  ...
  document.getElementById("headerTitle").innerText="Panel Administrativo";
}

function showCustomers(){
  document.getElementById("dashboard").style.display="none";
  document.getElementById("customersModule").style.display="block";
  document.getElementById("headerTitle").innerText="Clientes";
  fetchCustomers();
}
...
function renderCustomerTable(data){
  if(data.length===0){document.getElementById("customerTable").innerHTML="<p>No hay clientes.</p>";return;}
...
async function saveCustomer(){
  ...
  if(res.ok){showToast("Cliente creado correctamente");closeCustomerModal();fetchCustomers();}
...
loadDashboard();
      ...
      if(url.pathname==="/api/customers"){
        if(request.method==="GET"){
          const items=await env.DB.prepare("SELECT * FROM customers ORDER BY id DESC").all();
          return json(items.results);
        }
        if(request.method==="POST"){
          const data=await request.json();
          await env.DB.prepare("INSERT INTO customers(full_name,document_id,phone,email) VALUES(?,?,?,?)")
            .bind(data.full_name,data.document_id,data.phone,data.email)
            .run();
