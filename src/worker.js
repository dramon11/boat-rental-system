...
async function fetchJSON(path,options){const res=await fetch(path,options);return res.json()}
...
async function loadModule(type){
  ...
  container.innerHTML = \`
    <div class="module-header">
      <div><h2>\${type.charAt(0).toUpperCase()+type.slice(1)}</h2></div>
      <div class="actions">
    ...
    <div id="\${type}Modal" class="modal-overlay">
      <div class="modal">
        <h3>Nuevo \${type.charAt(0).toUpperCase()+type.slice(1)}</h3>
        <input id="\${type}_name" placeholder="Nombre"/>
        <input id="\${type}_extra" placeholder="Documento / Tipo"/>
      ...
      for(const e of entities){
        if(url.pathname===\`/api/\${e}\`){
          if(request.method==="GET"){
            const {results}=await env.DB.prepare(\`SELECT * FROM \${e} ORDER BY id DESC\`).all();
            return json(results);
          }
