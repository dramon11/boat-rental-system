  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/" && request.method === "GET") {
        ...
        'function showToast(msg,type){var t=document.getElementById("toast");t.innerText=msg;t.className="toast show "+type;setTimeout(function(){t.className="toast"},3000)}' +
        'async function fetchJSON(path,opt){var r=await fetch(path,opt);return r.json()}' +

        // DASHBOARD
        'async function showDashboard(){var c=document.getElementById("mainContent");c.innerHTML=' +
        '"<div class=\'cards\'><div class=\'card\'><h4>Ingresos Hoy</h4><h2 id=\'income\'>$0</h2></div>"+' +
        ...
        '"<div class=\'charts\'><div class=\'chart-box\'><h4>Resumen General (Barras)</h4><canvas id=\'barChart\'></canvas></div>"+' +
        ...
        'var values=[data.income_today,data.active_rentals,data.available_boats,data.total_customers];' +
        ...
        // CRUD genérico
        'async function loadModule(type){var c=document.getElementById("mainContent");c.innerHTML=' +
        '"<div class=\'module-header\'><div><h2>"+type+"</h2></div>"+' +
        '"<div class=\'actions\'><input id=\'searchInput\' class=\'input-search\' placeholder=\'Buscar...\' />"+' +
        ...
        '"<div id=\'"+type+"Modal\' class=\'modal-overlay\'><div class=\'modal\'>"+' +
        '"<h3>Nuevo "+type+"</h3>"+' +
        '"<input id=\'"+type+"_name\' placeholder=\'Nombre\' /><input id=\'"+type+"_extra\' placeholder=\'Documento / Tipo\' />"+' +
        '"<div style=\'margin-top:10px\'><button class=\'btn-success\' onclick=\'saveItem(\\\""+type+"\\\")\'>Guardar</button>"+' +
        '"<button class=\'btn-secondary\' onclick=\'closeModal(\\\""+type+"\\\")\'>Cancelar</button></div></div></div>";' +
        'await fetchItems(type);}' +

        'async function fetchItems(type){document.getElementById("loader").style.display="block";' +
        'var data=await fetchJSON("/api/"+type);document.getElementById("loader").style.display="none";renderTable(type,data);' +
        'document.getElementById("searchInput").addEventListener("input",function(e){var val=e.target.value.toLowerCase();renderTable(type,data.filter(function(i){return (i.full_name||i.name||\'\').toLowerCase().includes(val);}));});}' +

        'function renderTable(type,data){if(data.length===0){document.getElementById("table").innerHTML="<p>No hay datos</p>";return;}' +
        'var html="<table class=\'data-table\'><thead><tr><th>Nombre</th><th>Extra</th><th>Acciones</th></tr></thead><tbody>";' +
        'data.forEach(function(i){html+="<tr><td>"+(i.full_name||i.name)+"</td><td>"+(i.document_id||i.extra||"-")+"</td>"+' +
        ...
        'async function saveItem(type){var body={full_name:document.getElementById(type+"_name").value,document_id:document.getElementById(type+"_extra").value};' +
        'var res=await fetch("/api/"+type,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});' +
        'if(res.ok){showToast(type+" creado correctamente","success");closeModal(type);fetchItems(type);} else{showToast("Error al crear "+type,"error");}}' +

        'async function deleteItem(type,id){if(!confirm("Eliminar?"))return;await fetch("/api/"+type+"/"+id,{method:"DELETE"});showToast(type+" eliminado","success");fetchItems(type);}' +
      ...
      for(var i=0;i<entities.length;i++){
        ...
        if(url.pathname=="/api/"+e){
          if(request.method==="GET"){
            var all=await env.DB.prepare("SELECT * FROM "+e+" ORDER BY id DESC").all();
            return json(all.results||[]);
          }
          if(request.method==="POST"){
            var body=await request.json();
            await env.DB.prepare("INSERT INTO "+e+" (full_name,document_id) VALUES (?,?)")
