...
<body>
  ...
  <script>
      ...
      fetch('/api/customers')
        ...
        .then(data => {
          ...
          data.forEach(c => {
            ...
            option.value = c.id;
            option.textContent = c.full_name + (c.document_id ? ' (' + c.document_id + ')' : '');
      ...
      fetch('/api/boats')
        ...
        .then(data => {
          ...
          data.forEach(b => {
            const option = document.createElement('option');
            option.value = b.id;
            option.textContent = b.name + ' (' + (b.type || 'Sin tipo') + ')';
    ...
    function editReservation(btn) {
      ...
      // Cargar datos actuales (opcional - por ahora solo abre el modal)
