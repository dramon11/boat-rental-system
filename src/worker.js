  ...
  <script>
    ...
    async function openReservationModal() {
      ...
      try {
        ...
        customers.forEach(c => options += \`<option value="\${c.id}">\${c.full_name}</option>\`);
      ...
      try {
        ...
        boats.forEach(b => options += \`<option value="\${b.id}">\${b.name}</option>\`);
    ...
    function renderInvoicesTable(data) {
      ...
      data.forEach(i => {
        html += \`<tr>
          ...
          <td><button class="btn btn-success" onclick="editInvoice(\${i.id})">Editar</button></td>
        </tr>\`;
      });
      html += '</tbody></table>';
      el.innerHTML = html;
